...
29. 重复元字符
    *       0个或多个匹配
    ...
    匹配连在一起的任意四位数字: select id,name from table_name where id regext '[[:digit:]]{4}';
...
37. 使用having来进行对分组进行过滤,例如:select id,sum(score) from table_name group by id having sum(score) > 120;
